...
- **Nichts veraltet von selbst.** Der einzige Weg, an dem sich etwas ändert, ist „Von xjustiz.de aktualisieren": Versionsliste neu holen, dann die bereits **geholten** Versionen ersetzen. Eine nur gelistete Version wird erst heruntergeladen, wenn jemand sie im Umschalter wählt — der Speicher wächst nicht ungefragt.
- **Nachtrag 26.08.25:** Gespeichert wird nicht erst die geholte Version, sondern schon die **Bezugsquelle** — `PUT /api/schemas/:id` **ohne** `files` merkt Kennung, Label und `zipUrl` (vorhandene Dateien bleiben unberührt). Sonst überlebte nur, was auch tatsächlich geladen wurde: wer die Liste bloß abrief, fand nach dem Neuladen wieder die beiden hinterlegten Versionen vor — der gemeldete Fall. Ein Eintrag ohne Dateien ist damit ein gültiger Zustand („bekannt, noch nicht geholt"), den Umschalter-Tooltip und Aktualisieren-Lauf unterscheiden müssen (`SchemaStoreService.hatDateien`).
