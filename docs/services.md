...
**Kardinalität wird hart durchgesetzt** (Issue #27): `kardSperreHinzu(path)` und `kardSperreEntfernen(path)` liefern den **Grund** der Sperre bzw. `null` — maßgeblich ist die effektive Kardinalität (`StateService.effKard`: Entscheidung, sonst Vorgabe, sonst Schema), die Begründung nennt über `minProfil`/`maxProfil` die Quelle der verletzten Grenze. Ein Element ohne eigene Ausprägungen zählt als **ein** Vorkommen (der generische Unterbaum), nicht als null. Beides greift nur im Instanz-Modus — beim Profilieren sind Ausprägungen ein Entwurfsmittel und bleiben frei. Baum (`⧉`, `✕`, „+ Vorkommen") und Detailbereich („+ weiteres Vorkommen", `⧉` in der Vorkommen-Liste) sperren daran und nennen den Grund. Jeder Weg, der ein Vorkommen **anlegt**, prüft doppelt: `[disabled]` am Knopf und ein Guard in der Aktion — sonst hinge die Sperre allein an der Darstellung.
