...
Daraus folgt, was beim Start materialisiert wird: als Ausprägung erst eine Mindestanzahl >= 2 (`TestmessageCreateService.legeMindestVorkommenAn`) — eine Mindestanzahl 1 erfüllt das Element selbst, sobald es in der Nachricht ist. Damit sie mehr ist als eine Zählregel, wird sie über `kardSperreWeglassen` durchgesetzt: ein Element, dessen **Profil**-Mindestanzahl >= 1 ist, lässt sich nicht weglassen (`setzeAufnahme` verweigert es wie bei zwingend gesetzten Elementen). Maßgeblich ist allein die Eingrenzung der Profilierung — die Mindestanzahl des Schemas macht ein Element ohnehin zum Pflicht-Rückgrat ohne Aufnahme-Frage, und ein Auswahl-Zweig trägt sein `min=1` aus dem Schema; ihn zu sperren machte den Zweigwechsel unmöglich.
