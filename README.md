...
Werkzeug zur Visualisierung von XJustiz-Nachrichten und zur Erstellung von Profilierungen (Kommunikationsszenarien) — auch für die gemeinsame Arbeit mit Nicht-Technikern. Eine einzelne HTML-Datei, läuft ohne Installation im Browser (`Profilierer.html` per Doppelklick öffnen).
...
3. **Profilieren** — Kasten anklicken, rechts im Detailbereich entscheiden:
   ...
   - **Codelisten-Werte** einschränken (inline gepflegte Listen per Checkbox). Für externe Listen (Code-Typ 3) gibt es drei Wege: **„Codelisten: XRepository"** ruft per REST-Schnittstelle alle vom Standard genutzten Listen ab (`/version_standard/…/genutzteAktuelleCodelisten`, Typ 3 in aktuell gültiger Version); im Detailbereich lässt sich eine **einzelne Liste** nachladen; oder **„Codelisten: Datei…"** liest Genericode-XML/ZIP aus lokalen Dateien. Geladene Listen werden gemäß XRepository-Nutzungsbedingungen im Browser **gecacht** und stehen beim nächsten Start sofort bereit. Hinweis: Das XRepository sendet keine CORS-Freigabe, daher scheitert der Direktabruf im Browser. **Zuverlässigste Lösung:** das Tool über den beiliegenden Start-Helfer öffnen — `python3 xrep-proxy.py` im Projektordner (serviert die App unter `http://localhost:8737` und reicht die XRepository-Aufrufe same-origin durch; nur Python-Standardbibliothek, öffnet den Browser automatisch). Ohne Helfer versucht das Tool — einmalig mit Zustimmung — öffentliche Weiterleitungsdienste (codetabs.com, corsproxy.io, allorigins.win; nicht immer verfügbar). Es bleibt außerdem der ZIP-Download-Link plus „Codelisten: Datei…". Werte sind einsehbar, ankreuzbar, filterbar und fließen in die Beispiel-XML ein (inkl. `listVersionID`).
...
- Für den Excel-Export ist eine Internetverbindung nötig (SheetJS via CDN); alles andere läuft offline.
